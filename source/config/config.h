...
public:
  ...
  /**
   * @brief Creates a @a Config instance from an XML file.
   */
   ...
   * @param v
   */
  Bool getConfigValue(const std::string &name, const std::string &type, Config::Appearance mustSet,
                      const std::string &defaultValue, const std::string &annotation, Double &v);
   ...
   * Every program and class will be called.
...
protected:
  ...
  public:
    ...
    XmlNodePtr   xmlNode;
    /// last processed child
    XmlNodePtr   xmlLastChild;
    LoopPtr      loopPtr;
    // varList without loop variables
    VariableList loopVarListOld;
 ...
 * @param var Variable for configuration of the tag @a name
 * @param mustSet
 * @param defaultValue Default value for this variable.
 * @param annotation Description of this variable.
 *
 * @note If @a name is found in @a config, its content will be set in @a var and TRUE is returned.
 * If @a name is not found in @a config or its content is empty, FALSE is returned. In this case,
 * the value of @a var depends on the specified @a mustSet:
 * - @a mustSet = Config::MUSTSET, an exception will be thrown.
 * - @a mustSet = Config::OPTIONAL, @a var will be left untouched and @a defaultValue is not used (@a defaultValue might be set as a hint for groopsGui).
 * - @a mustSet = Config::DEFAULT, @a var will be set to @a defaultValue (or in case an empty class is created).
 * @ingroup configGroup
 */
